...
export function fromPartialNullable<K extends string>(
  property: K
): <A>(
...
export const silent: d.Decoder<
  ...
  O.Option<string>
> = pipe(fromPartialFlag("silent"), d.map(O.map(() => "--silent")))

export const verbose: d.Decoder<
  Partial<Record<"verbose", true>>,
  O.Option<string>
> = pipe(fromPartialFlag("verbose"), d.map(O.map(() => "--verbose")))
...
export const logging = d.union(silent, verbose, logLevel, enableChromeLogs)
